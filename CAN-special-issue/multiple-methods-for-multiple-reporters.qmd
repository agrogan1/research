...
We first run a latent class analysis to generate latent underlying classes based upon the independent variables. 

The number of latent classes is determined by running models with different numbers of latent classes, and comparing those models using *fit statistics*, and *likelihood ratio tests*. 

```{stata}
#| eval: false

gsem(P SR administrative covariates <-, gaussian), (lclass(C 3))
