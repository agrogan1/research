...
csl: apa.csl
nocite: |
  @McLaughlinKatieA2020, @Machlin2025
---

# Background

::: {.callout-tip}
## Created With `Statamarkdown`

This document was created using the amazing package by @Hemken, which should be cited in the final version of the paper. 
:::
...
use "/Users/agrogan/Library/CloudStorage/Dropbox-UniversityofMichigan/Andrew Grogan-Kaylor/Lehigh measurement and causal inference analyses/Data sets/Stata data/Lehigh_working_concept_paper.dta", clear

set linesize 96 // set linesize for Quarto

global covariates "i.gender age i.ethrace i.marital annual_income" // set global macro
...
::: {#nte-key-ideas-questions .callout-note}
## Key Ideas or Questions

* Many sources in the literature note that data can come from multiple reporters. It is often suggested that it is important to *triangulate* these different sources of information, yet what is meant exactly, or operationally, by such *triangulation* is often unclear. 
* How do the different methods highlight, or not highlight:
    + The differences between prospective and retrospective reports, especially in predicting outcomes?
    + The level of agreement or disagreement between different reporters?
    + Issues of severity, chronicity, timing and developmental stages across reporters?
:::
...
dtable i.gender age i.ethrace i.marital annual_income annual_income PR1 PR2 SR1 SR2 i.administrative outcome
...
: Variables and Variable Labels {#tbl-variables .borderless .hover}
...
regress outcome PR1 PR2 SR1 SR2 administrative $covariates
...
gsem (outcome <- $covariates SR1 SR2 PR1 PR2 administrative) ///
  (SR2 <- SR1) ///
  (PR2 <- PR1), ///
  cov(e.outcome*e.SR2*e.PR2) /// correlated errors
  nocapslatent // capital letters are NOT latent variables
